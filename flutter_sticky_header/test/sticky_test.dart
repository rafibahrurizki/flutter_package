...
void main() {
  ...
  testWidgets('Mix sticky and not sticky headers - reverse', (
    ...
    // Header #1 is in the tree (because the sliver is onstage).
    expect(
      tester.getRect(header01Finder),
      const Rect.fromLTRB(0, 800, 400, 880),
    );
    expect(header02Finder, findsNothing);
  });
...
class _HierarchyHeader extends StatelessWidget {
  const _HierarchyHeader({Key? key, required this.hierarchy}) : super(key: key);
...
class _Sliver100 extends StatelessWidget {
  const _Sliver100({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return SliverToBoxAdapter(child: SizedBox(height: 100));
  }
}
...
class _Sliver extends StatelessWidget {
  const _Sliver({Key? key}) : super(key: key);
...
class _SliverItem extends StatelessWidget {
  const _SliverItem({Key? key}) : super(key: key);
